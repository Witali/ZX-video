...
- **Цель/база:** предложение пользователя о другом формате, ветка
  `codex/vector-contours` от `2b3b4ce`. Полные 4221 кадр без титров по
  `movie_no_credits.json`, включая сцену после титров, прежние разрешение,
  состояния и 25326 AY ticks. База FAP3: 3097959 байт до ZX0,
  1932429 после ZX0 с заголовками блоков. Это исследование данных на ПК.
- **Первая попытка:** собственный обход углов пикселей с отверстиями и
  диагональными касаниями в существующей сетке 128×72 до дизеринга;
  native-область остаётся 256×144. Целочисленные полигоны с допуском
  0,5/1 точки, точные растровые исправления и атрибуты. Гибрид выбирает
  меньшую запись до ZX0: полигоны / прямоугольники изменений к n−2 / растр.
  Чистая сетка — 23375323 байт, полигоны — 19226193, гибрид — 7453649.
  В гибриде 69 полигональных кадров, 2357 с прямоугольниками, 1795 растровых.
- **Вторая попытка:** сеточное ребро записано байтом направления/длины,
  соседние вершины — короткими dx/dy с выходом в абсолютные координаты.
  Геометрия и исправления прежние. Чистая сетка — 15064969 байт,
  полигоны — 15940981, гибрид — 7396678. Выбраны 182 сеточных кадра,
  48 полигональных, 2300 с прямоугольниками и 1691 растровый.
- **Настоящий ZX0:** optimal v2, 8192 байта на блок, тот же encoder, что у
  базы; каждый блок независимо распакован. Первый гибрид: 910 блоков,
  **3468203 байта (+79,47%)**. Второй: 903 блока,
  **3476798 байт (+79,92%)**. Уменьшение сырой записи на 56971 байт
  увеличило ZX0 на 8595. DEFLATE в покадровых отчётах — только быстрый
  предварительный отбор, не результат ZX0. Оба варианта требуют минимум
  шесть целых TRD даже без накладных расходов; рабочие образы не собирались.
- **Качество/полнота:** все 4221 кадр каждого представления, атрибуты и
  исходные AY-записи восстановлены точно; покадровые ошибки нулевые.
  Четыре теста прошли, включая отверстия, тонкие линии, случайные маски,
  полуоткрытые границы и недопустимое отрицательное смещение вершины.
  Просмотрены кадры 12, 408, 2858, 3009, 3686, 3685. Правая колонка
  контактного листа показывает ошибки до исправлений, а не конечный кадр.
- **Расходы/ограничения:** в среднем 1150,23 вершины на полигональный кадр
  и 504,30 исправляемой точки. Счётчик записей native-байтов с очисткой:
  в среднем 8964,59, максимум 11298; это не CPU T-states. Наибольшая
  полигональная запись 10400 байт превышает старое окно 4704. Hot path
  плеера не менялся, разница 0 T. Потребитель Z80, его абсолютные такты,
  размещение новых таблиц в 128 КиБ, ROM/ULA/диск и полный темп не проверены.
- **Решение:** сохранить обе попытки в отдельной ветке; базу и корневые
  TRD оставить. Проверенный формат проигрывает по данным, переносить его
  в плеер сейчас нецелесообразно. Это не доказательство против всех
  векторных подходов: следующий отдельный опыт — ID фигур, перенос/дельты
  вершин между кадрами и векторы только для выгодных областей в сочетании
  с текущими растровыми дельтами. Готовый кадр строить прямо в скрытом
  экранном банке с учётом очистки старого положения фигуры, без полной копии.
- **Материалы:** [формат, результаты и дальнейший план](toolkit/INTEGER_VECTOR_FORMAT_ru.md),
  [код](toolkit/integer_contours.py), [первый измеритель](toolkit/probe_integer_contours.py),
  [компактная запись](toolkit/probe_compact_contours.py),
  [первые кадры/качество](toolkit/integer_contours_report.json),
  [вторые кадры/качество](toolkit/compact_contours_report.json),
  [первый ZX0](toolkit/integer_contours_hybrid_zx0.json),
  [второй ZX0](toolkit/compact_contours_hybrid_zx0.json),
  [сводка](toolkit/compact_contours_summary.json),
  [сцены](toolkit/integer_contours_preview.png).
