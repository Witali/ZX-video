...
def harness(tables,mapping,compiled=False):
    options=dict(OPTIONS,skip_static_stripes=False)
    return Harness(tables,mapping,carry_huffman=True,register_fragments=True,
        cached_huffman_byte=compiled,metadata_mode='compiled' if compiled else 'standard',**options)


class UncontendedFrameTests(unittest.TestCase):
    def test_every_presence_pattern_in_relocated_mask_pages(self):
        self.check_presence_patterns(False)

    def test_compiled_patterns_in_relocated_mask_pages(self):
        self.check_presence_patterns(True)

    def check_presence_patterns(self,compiled):
        h=harness([bytes([8]*256)]*2,bytes(256),compiled);move.install_stage(h);c=h.cpu
        if compiled:c.port_7ffd=0x17
        for mask in range(256):
            ...
            result=h.execute(h.metadata_entry)
            self.assertEqual(result['total_tstates'],h.metadata_formula(encoded))
    ...
    def test_mixed_frames_and_ay_after_every_instruction(self):
        self.check_mixed_frames(False)

    def test_compiled_cached_frames_and_ay_after_every_instruction(self):
        self.check_mixed_frames(True)

    def check_mixed_frames(self,compiled):
        ...
        old,new=[harness(tables,mapping,compiled) for _ in range(2)];move.install_stage(new)
