...
class FrameStreamCPU(stream.StreamCPU):
    ...
    def write8(self, address, value):
        if self.guarding:
            ...
            if self.phase in ('metadata', 'reconstruct', 'output', 'handoff', 'attribute_groups'):
...
class Harness:
    def __init__(self, ring, tables, mapping, frames, *, ring_start=0xfff0, bulk=False, zero_copy=False, skip_noop_runs=False,
                 ...
                 unrolled_copy=False,unrolled_cache=False,attribute_groups=False,attribute_flags=False,gray_cells=False,early_ay=False,sparse_patches=False,disk_refill_entry=None,inline_matches=False,fast_noop_scan=False,irq_safe_paging=False,static_cache_borders=False,carry_huffman=False):
        ...
        f = self.frame = pipeline.Harness(tables, mapping, raw_attributes=True, decode_metadata=True,
            ...
            attribute_flags=attribute_flags,gray_cells=gray_cells,sparse_patches=sparse_patches,fast_noop_scan=fast_noop_scan,static_cache_borders=static_cache_borders,carry_huffman=carry_huffman)
        ...
        code, bridge, self.p, listing = builder.build(self.z, self.r, f.w, f.draw, f.metadata_labels, self.audio,
            **({'stored_guards':stored_guards,'separate_prepare':packet_ahead,'early_ay':early_ay} if bulk else {}),
    ...
    def execute(self, entry, *, interrupt=None):
        ...
        while cpu.pc != stream.STOP:
            ...
            if self.bulk and pc == self.audio['audio_enqueue_six']:
                ...
                cpu.packet_end = pipeline.INPUT+EARLY_AY_BYTES if self.early_ay else word(cpu,self.p['payload_end'])
            if self.bulk and pc == self.p['video_payload_ready']:
