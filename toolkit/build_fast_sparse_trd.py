...
def main() -> None:
    ...
    output_metadata = {
        ...
        "direct_input_model": {
            ...
            "slice_begin_active_delta_tstates": 154,
            "slice_resume_active_delta_tstates": 148,
        ...
        "cycle_model": {
            ...
            "bitmap_rle": "215+50*L+155*R+90*NL+39*NR+sum(DL(n))+sum(DR(n))" if fast_draw else "208+95*L+152*R+183*NL+167*NR",
            "bitmap_rle_symbols": "L/R: literal/repeat token counts; NL/NR: corresponding output lengths, total 32",
            "bitmap_rle_literal_delta": "DL(1)=-2; DL(even n)=22-45*(n//2); DL(odd n>1)=4-45*(n//2)" if fast_draw else None,
            "bitmap_rle_repeat_delta": "DR(even n)=16-21*(n//2); DR(odd n)=-2-21*(n//2)" if fast_draw else None,
