...
def emit(a, *, memory_clock=False):
    ...
    a.label('seek_side_return');a.emit(0xFB if memory_clock else 0xF3)
    ...
    a.label('seek_return');a.emit(0xFB if memory_clock else 0xF3)
    playback_schedule.select_rom_clock(a,False,memory_clock)
    a.emit(0x3E,0x84,0x32);a.word(0x5CFE)
    a.emit(0xE1,0xC9)


def emit_keepalive(a, *, memory_clock=False):
    ...
    a.label('keepalive_seek');a.emit(0xFB if memory_clock else 0xF3)
    playback_schedule.select_rom_clock(a,True,memory_clock)
    a.emit(0x06,0)
    ...
    a.label('keepalive_seek_return');a.emit(0xFB if memory_clock else 0xF3)
    playback_schedule.select_rom_clock(a,False,memory_clock)
    if memory_clock:
        a.abs16(0x2A,'elapsed_fields');a.abs16(0x22,'last_disk_fields');a.emit(0xFB,0xC9)
    else:
