"""Cold-play three exact TRDs with demand-sized partial ZX0 consumption."""
import argparse
import json
...
def main():
    ...
    p.add_argument('--mode',choices=('baseline','demand'),default='demand')
    args=p.parse_args()
    for part in (1,2,3):
        ...
        command=[sys.executable,str(Path(__file__).with_name('measure_fap3_fuse.py')),
            ...
            '--slot-queue','--compiled-masks','--cached-huffman-byte','--inline-literals','--trace-pipeline','--fixture-zx0',str(args.zx0)]
        if args.mode=='demand':command.append('--demand-decode')
