...
def cpu_profile(builder, start, end):
    ...
    h = player_harness(ring, builder.tables, builder.mapping, end-start, disk_reader=False,inline_matches=builder.inline_matches,
        fast_noop_scan=builder.fast_noop_scan,irq_safe_paging=builder.irq_safe_paging)
    if start:
    ...
    return dict(complete=True, frame_start=start, frame_end_exclusive=end, frames=end-start,
        ...
        unchanged_player_baseline=None if builder.inline_matches or builder.fast_noop_scan or builder.irq_safe_paging else '00313d3',
        player_hot_path_delta_tstates=None if builder.inline_matches or builder.fast_noop_scan or builder.irq_safe_paging else 0,
...
def main():
    ...
    builder = Builder((output/'work/stream.raw').read_bytes(), states,
        ...
        irq_safe_paging=first.get('irq_safe_paging',False))
