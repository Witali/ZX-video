...
def cpu_profile(builder, start, end):
    ...
    h = player_harness(ring, builder.tables, builder.mapping, end-start, disk_reader=False,inline_matches=builder.inline_matches,
        fast_noop_scan=builder.fast_noop_scan,irq_safe_paging=builder.irq_safe_paging,static_cache_borders=builder.static_cache_borders,carry_huffman=builder.carry_huffman,register_fragments=builder.register_fragments,cached_huffman_byte=builder.cached_huffman_byte)
    if start:
    ...
    return dict(complete=True, frame_start=start, frame_end_exclusive=end, frames=end-start,
        ...
        unchanged_player_baseline=None if builder.inline_matches or builder.fast_noop_scan or builder.irq_safe_paging or builder.static_cache_borders or builder.carry_huffman or builder.register_fragments or builder.cached_huffman_byte else '00313d3',
        player_hot_path_delta_tstates=None if builder.inline_matches or builder.fast_noop_scan or builder.irq_safe_paging or builder.static_cache_borders or builder.carry_huffman or builder.register_fragments or builder.cached_huffman_byte else 0,
        ...
        cached_huffman_byte=builder.cached_huffman_byte,
...
def verify_volumes(builder, records, output, fuse=None, timeout=1800):
    ...
    report = dict(complete=False, all_nominal_deadlines_met=False, release=False,
        ...
        player_hot_path_changed=builder.inline_matches or builder.fast_noop_scan or builder.irq_safe_paging or builder.static_cache_borders or builder.carry_huffman or builder.register_fragments or builder.cached_huffman_byte,
        player_hot_path_delta_tstates=None if builder.inline_matches or builder.fast_noop_scan or builder.irq_safe_paging or builder.static_cache_borders or builder.carry_huffman or builder.register_fragments or builder.cached_huffman_byte else 0, disks=[])
...
def main():
    ...
    builder = Builder((output/'work/stream.raw').read_bytes(), states,
        ...
        irq_safe_paging=first.get('irq_safe_paging',False),static_cache_borders=first.get('static_cache_borders',False),carry_huffman=first.get('carry_huffman',False),register_fragments=first.get('register_fragments',False),cached_huffman_byte=first.get('cached_huffman_byte',False))
