"""Pair full queue consumption with/without literal CALL/RET on all exact TRD bytes.

One initial prefill, no background refill. Counts every Z80 instruction,
copy and paging operation; ROM services are mocked. No rendering, IRQ/ULA,
physical disk latency or release claim. Same tokens, quotas and ownership.
"""
import argparse
...
def main():
    ...
    args=p.parse_args()
    report=dict(complete=False,release=False,scope=__doc__,baseline_commit='89a56ed',
    ...
    for part in (1,2,3):
        ...
        hs={name:QueueHarness(Producer(image,meta['video_start_sector'],meta['video_sectors'],inline_literals=inline),len(blocks))
            for name,inline in (('baseline',False),('inline',True))}
        ...
        row=dict(part=part,trd_sha256=sha(image),stream_sha256=sha(stream),raw_sha256=sha(raw),
            ...
            literal_tokens=literal_tokens,expected_delta_tstates=-27*literal_tokens-17*len(blocks),
        ...
        for index in range(meta['frames']):
            ...
            if frame['baseline']['sectors']!=frame['inline']['sectors']:raise AssertionError('read schedule changed')
            frame['delta_tstates']=frame['inline']['tstates']-frame['baseline']['tstates']
        ...
        measured=row['summary']['inline']['tstates']-row['summary']['baseline']['tstates']
        if measured!=row['expected_delta_tstates']:raise AssertionError(('unexpected complete delta',measured,row['expected_delta_tstates']))
    ...
    report.update(complete=True,checked_frames=sum(len(v['frames']) for v in report['volumes']),
        totals={name:sum(v['summary'][name]['tstates'] for v in report['volumes']) for name in ('baseline','inline')},
