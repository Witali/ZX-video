"""FAP2 Z80 parser: read a whole packet, consume values in place."""
...
def build(zx0,reader,wrapper,draw,metadata,audio):
    ...
    addr('LD HL,(length)',0x2a,LENGTH,16); addr('LD DE,296',0x11,296,10)
    emit('OR A',[0xb7],4); emit('SBC HL,DE',[0xed,0x52],15); addr('JP C,fatal',0xda,zx0['fatal'],10)
    addr('LD DE,window-295',0x11,INPUT_END-INPUT-295,10)
    ...
    addr('LD DE,cache+vectors+map+guards',0x11,277,10); emit('ADD HL,DE',[0x19],11)
    ...
    emit('INC HL',[0x23],6); addr('LD (literal_pointer),HL',0x22,wrapper['literal_pointer'],16)
    emit('EX DE,HL',[0xeb],4); addr('LD HL,(payload_end)',0x2a,'payload_end',16)
    emit('DEC HL',[0x2b],6); emit('LD A,(HL)',[0x7e],7); emit('OR A',[0xb7],4)
    addr('JP NZ,fatal',0xc2,zx0['fatal'],10); emit('SBC HL,DE',[0xed,0x52],15)
